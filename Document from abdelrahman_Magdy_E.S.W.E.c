/*
 ...
 PIN (PORT_D5) ENABLE PIN STRPPER MOTORS DRIVERS
...
void main() {
  float vol,x,x2,x3,Length,DELAY_PULSE_I,DELAY_PULSE_E,ITOE,IEPerM
  ,counter_pulse,er[2];
...
{ 
  ...
  IEPerM=adc_read(2);
     ...
     LCD_chr(3,9,f1[1]);
     LCD_chr(3,10,f1[2]);
     DELAY_PULSE_E=(((60/IEPerM)/2)*15)-((er[1]*(((60/IEPerM)/2)*15))/100);
     DELAY_PULSE_i=(DELAY_PULSE_E/itoe);
     DELAY_PULSE_E=DELAY_PULSE_E+(DELAY_PULSE_E-DELAY_PULSE_i);
     x3=x2;
  while(1)
  {
            //||
    while(portd.B7==1||portd.B6==1)      //<<<<<<<<<<E
    {
     ...
     {
       counter_pulse=counter_pulse+1;        //delay_in_us;
     }
     counter_pulse=0;
     portc.b1=0;
     portc.b2=0;
    ...
    while(x<=x3)                      //<<<<<<<<<<I
    {
     ...
     while(counter_pulse<=DELAY_PULSE_i)
     {
       counter_pulse=counter_pulse+1; //delay_us(1);
     }
     counter_pulse=0;
     x=x+1;
   }
